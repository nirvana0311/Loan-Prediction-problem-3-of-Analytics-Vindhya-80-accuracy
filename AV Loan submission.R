...
training_copy=read.csv("C:\\Users\\nirva\\IdeaProjects\\R\\Loan Prediction\\train.csv")
test_copy=read.csv("C:\\Users\\nirva\\IdeaProjects\\R\\Loan Prediction\\test.csv")

training=training[,!(colnames(training) %in% 'Loan_ID')]
test=test[,!(colnames(test) %in% 'Loan_ID')]
...
first_model=train(Loan_Status~.,method='rf',data = training,trControl=control,tuneGrid=tunegrid,ntree=2501)
first_model
table(training$Loan_Status,predict(first_model))
plot(first_model)


aggr(test,col=c('blue','red'),numbers=T,sortVars=T,labels=names(test),cex.axis=.7,gap=3,  ylab=c("Histogram of missing data","Pattern"))

apply(test,2,pMiss)

tempdataTest=mice(data = test,m = 1,method = 'pmm',maxit = 50,seed = 523)
densityplot(tempdataTest)
test=complete(tempdataTest,1)




sum(is.na(test))
Submission=predict(first_model,newdata = test)
test_copy$Loan_Status=Submission
Submission_1=write.csv(x = test_copy,file = 'Submission_1.csv')
getwd()
